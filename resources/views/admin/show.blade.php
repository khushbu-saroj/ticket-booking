    ...
    <h1 class="text-3xl font-semibold">Create Show</h1>
            @if (session('success'))
        <div class="p-4 mb-4 text-green-700 bg-green-100 border border-red-400 rounded">
            {{ session('success') }}
        </div>
    @endif
        <form action="{{ url('/show_save') }}" method="POST" class="space-y-4">
            @csrf
        
            <div>
                <label for="theater" class="block text-sm font-medium text-gray-600">Select Theater</label>
        ...
        @foreach($theaters as $theater)
            <option value="{{$theater->id}}">{{$theater->theatername}}</option> 
            @endforeach
        </select><br>

        <label for="theater" class="block text-sm font-medium text-gray-600">Select Movie</label>
            ...
            <label for="datetime" class="block text-sm font-medium text-gray-600">Date Time</label>
